...
/**
 * This files contains some configurations that you might want to have in your project. Some
 * configurations are made in for the sake of demonstration and not required for the library
 * functioning. Usually you can have even more minimalistic config
 *
 * Take into consideration that we autoscan files searching for Aggregates, Events and
 * StateTransition functions. Autoscan enabled via [event.sourcing.auto-scan-enabled] property.
 *
...
class EventSourcingLibConfiguration {
    ...
    @Autowired private lateinit var eventSourcingServiceFactory: EventSourcingServiceFactory

    @Autowired private lateinit var eventStreamManager: AggregateEventStreamManager

    /** Use this object to create/update the aggregate */
    @Bean
    fun paymentsEsService() =
